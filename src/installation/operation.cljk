(ns installation.operation
  "OperationActor -- one industrial-machinery-installation-project
  coordination operation = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (Installation Advisor) is sealed
  into a single node (:advise); its proposal is ALWAYS routed through the
  Installation Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one installation-coordination operation (intake ->
  ...
  :rejected). `:schedule-installation-operation`/`:flag-safety-concern`
  ALWAYS reach this node -- see `installation.phase`/`installation.
  governor` ns docstrings. `:log-installation-record` and `:order-
  supplies` (below the cost threshold) MAY auto-commit at phase 3 when
  the governor is clean -- see `installation.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `installation.
  governor` ns docstring) -- the `:commit` node writes a COORDINATION
  ARTIFACT to the SSoT + audit ledger, NEVER a real-world actuation.
  ONLY when the committed op is `:flag-safety-concern` does `:commit`
  ADDITIONALLY send the safety-concern notice (mail + phone,
  `installation.notify`) to the site's `:safety-contacts` roster -- always
  AFTER a human has approved it, since `:flag-safety-concern` is never
...
(defn build
  ...
  opts:
    :advisor      -- an `installation.advisor/Advisor` (default: mock-advisor)
    :notifier     -- an `installation.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Installation Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Installation Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
