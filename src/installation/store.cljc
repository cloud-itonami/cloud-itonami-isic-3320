(ns installation.store
  "SSoT for the industrial-machinery-installation-coordination actor,
  behind a `Store` protocol so the backend is a swap, not a rewrite -- the
  same seam every prior `cloud-itonami-isic-*` actor in this fleet uses:
  ...
  (test/installation/store_contract_test.clj), which is the whole point:
  the actor, the Installation Governor and the audit ledger never know
  ...
  counter: `:log-installation-record` (installation-record-log),
  `:schedule-installation-operation` (schedule-proposal), `:flag-safety-
  concern` (safety-concern-flag) and `:order-supplies` (supply-order-
  proposal). UNLIKE every sibling actor's `Store` that models a one-time
  double-actuation-guarded real-world event, none of these is that --
  every op may recur any number of times for the same site (a site gets
  logged repeatedly over its installation lifecycle, may be rescheduled,
  may accumulate multiple safety-concern flags, may place multiple supply
  orders) BECAUSE this actor never actually dispatches/authorizes/
  energizes/signs off on anything -- it only ever proposes, logs and
  schedules (`:effect :propose` unconditionally, see `installation.
  governor` ns docstring). The site's own `:site-verified?` / `:lift-
  plan-approved?` / `:safety-concern-unresolved?` / `:installation-
  notice-lead-days-actual` ground-truth fields are what the Installation
  Governor independently re-checks before a `:schedule-installation-
  operation` proposal may ever commit (see `installation.governor`).

  The ledger stays append-only on every backend: 'which site was logged,
  which installation-operation schedule was proposed, which safety
  ...
  immutable log."
  (:require [installation.registry :as registry]
...
(defprotocol Store
  ...
  (all-sites [s])
  (ledger [s])
  (installation-record-log-history [s] "the append-only installation-record-log history (installation.registry drafts)")
  (schedule-proposal-history [s] "the append-only installation-operation schedule-proposal history")
  ...
  (next-installation-record-sequence [s jurisdiction])
...
(defn demo-data
  "A small, self-contained installation-site set covering the happy path
  (site-1, JPN), the uncovered-jurisdiction / not-verified / lift-plan-
  ...
  []
  {:sites
   {"site-1" {:id "site-1" :name "Sakura Foods Plant -- Conveyor & Packaging Line Installation"
              :jurisdiction "JPN" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? false :safety-concern-unresolved? false
              :installation-notice-lead-days-actual 35
    ...
    "site-2" {:id "site-2" :name "Atlantis Refinery -- Compressor Skid Installation"
              :jurisdiction "ATL" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? false :safety-concern-unresolved? false
              :installation-notice-lead-days-actual 35
              :safety-contacts []
              :status :intake}
    "site-3" {:id "site-3" :name "鈴木製作所 プレス機設置工事"
              :jurisdiction "JPN" :site-verified? false :lift-plan-approved? false
              :rigging-hazard-detected? false :safety-concern-unresolved? false
              :installation-notice-lead-days-actual nil
              :safety-contacts [{:name "Sato (site supervisor)" :email "sato@example.com" :phone "+819000000003"}]
              :status :unverified}
    "site-4" {:id "site-4" :name "田中工業 クレーン据付工事"
              :jurisdiction "JPN" :site-verified? true :lift-plan-approved? false
              :rigging-hazard-detected? nil :safety-concern-unresolved? false
              :installation-notice-lead-days-actual 35
              :safety-contacts [{:name "Ito (site supervisor)" :email "ito@example.com" :phone "+819000000004"}]
              :status :lift-plan-pending}
    "site-5" {:id "site-5" :name "Liberty Ave Bottling Plant -- Filler Line Installation"
              :jurisdiction "JPN" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? true :safety-concern-unresolved? false
              :installation-notice-lead-days-actual 10
              :safety-contacts [{:name "Jordan (site supervisor)" :email "jordan@example.com" :phone "+15550000001"}]
              :status :notification-pending}
    "site-6" {:id "site-6" :name "ことぶき鋳造所 大型旋盤設置工事"
              :jurisdiction "JPN" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? true :safety-concern-unresolved? true
              :installation-notice-lead-days-actual 35
              ...
              :status :concern-open}
    "site-7" {:id "site-7" :name "Riverside Automotive Plant -- Stamping Press Installation"
              :jurisdiction "USA" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? false :safety-concern-unresolved? false
              :installation-notice-lead-days-actual 12
              :safety-contacts [{:name "Casey (site supervisor)" :email "casey@example.com" :phone "+15550000002"}]
              :status :ready-to-schedule}
    "site-8" {:id "site-8" :name "Alt-Maschinenfabrik -- Turbinen-Montage"
              :jurisdiction "DEU" :site-verified? true :lift-plan-approved? true
              :rigging-hazard-detected? false :safety-concern-unresolved? false
              :installation-notice-lead-days-actual nil
...
(defn- log-installation-record!
  ...
  (let [a (site s site-id)
        ...
        seq-n (next-installation-record-sequence s jurisdiction)
        result (registry/register-installation-record site-id jurisdiction seq-n)]
...
(defrecord MemStore [a]
  ...
  (ledger [_] (:ledger @a))
  (installation-record-log-history [_] (:installation-record-log @a))
  ...
  (next-installation-record-sequence [_ jurisdiction] (get-in @a [:installation-record-sequences jurisdiction] 0))
  ...
  (commit-record! [s {:keys [op path value]}]
    (let [site-id (first path)]
      (case op
        ...
        (let [{:keys [result jurisdiction site-patch]} (log-installation-record! s site-id value)]
          (swap! a (fn [state]
                     (-> state
                         (update-in [:installation-record-sequences jurisdiction] (fnil inc 0))
                         (update-in [:sites site-id] merge site-patch)
                         (update :installation-record-log registry/append result))))
          result)

        :schedule-installation-operation
        (let [{:keys [result]} (schedule-installation-operation! s site-id)
...
(defn seed-db
  ...
  (->MemStore (atom (assoc (demo-data)
                           :ledger []
                           :installation-record-sequences {} :installation-record-log []
...
(def ^:private site-spec
  ...
  {:id                                     {:attr :site/id}
   ...
   :lift-plan-approved?                    {:attr :site/lift-plan-approved? :coerce boolean}
   :rigging-hazard-detected?               {:attr :site/rigging-hazard-detected? :coerce boolean}
   :safety-concern-unresolved?             {:attr :site/safety-concern-unresolved? :coerce boolean}
   :installation-notice-lead-days-actual   {:attr :site/installation-notice-lead-days-actual}
...
(defrecord DatomicStore [conn]
  ...
  (installation-record-log-history [_] (ls/read-stream conn :installation-record-log/seq :installation-record-log/record))
  ...
  (next-installation-record-sequence [_ jurisdiction]
    (or (d/q '[:find ?n . :in $ ?j
              :where [?e :installation-record-sequence/jurisdiction ?j] [?e :installation-record-sequence/next ?n]]
  ...
  (commit-record! [s {:keys [op path value]}]
    (let [site-id (first path)]
      (case op
        :log-installation-record
        (let [{:keys [result jurisdiction site-patch]} (log-installation-record! s site-id value)
              next-n (inc (next-installation-record-sequence s jurisdiction))]
          (d/transact! conn
                       [(site->tx (assoc site-patch :id site-id))
                        {:installation-record-sequence/jurisdiction jurisdiction :installation-record-sequence/next next-n}])
          (ls/append-blob! conn :installation-record-log/seq :installation-record-log/record
                           (count (installation-record-log-history s)) (get result "record"))
          result)
