(ns installation.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura Foods Plant -- Conveyor & Packaging Line Installation" (:name (store/site s "site-1"))))
      (is (= "JPN" (:jurisdiction (store/site s "site-1"))))
      (is (true? (:site-verified? (store/site s "site-1"))))
      (is (true? (:lift-plan-approved? (store/site s "site-1"))))
      (is (false? (:safety-concern-unresolved? (store/site s "site-1"))))
      (is (= 35 (:installation-notice-lead-days-actual (store/site s "site-1"))))
      (is (false? (:site-verified? (store/site s "site-3"))) "site-3 seeded not-yet-verified")
      (is (false? (:lift-plan-approved? (store/site s "site-4"))) "site-4 seeded with an incomplete lift plan")
      (is (= 10 (:installation-notice-lead-days-actual (store/site s "site-5"))) "site-5 seeded with an insufficient JPN lead time")
...
(deftest write-and-ledger-parity
  (doseq [[label s] (backends)]
    (testing label
      (testing "log-installation-record commits a patch and appends an installation-record-log entry"
        (store/commit-record! s {:op :log-installation-record :path ["site-1"]
                                 :value {:id "site-1" :rigging-hazard-detected? true}})
        (is (true? (:rigging-hazard-detected? (store/site s "site-1"))))
        (is (= "Sakura Foods Plant -- Conveyor & Packaging Line Installation" (:name (store/site s "site-1"))) "unrelated field preserved")
        (is (= "JPN-INR-000000" (get (first (store/installation-record-log-history s)) "record_id")))
        (is (= 1 (store/next-installation-record-sequence s "JPN"))))
      (testing "schedule-installation-operation drafts a record and advances the sequence"
        (store/commit-record! s {:op :schedule-installation-operation :path ["site-1"]
      ...
      (testing "flag-safety-concern sets safety-concern-unresolved? true, drafts a record + notice document"
        (store/commit-record! s {:op :flag-safety-concern :path ["site-1"]
                                 :value {:site-id "site-1" :concern-description "test sling wear"}})
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-installation-record :path ["site-7"]
                                 :value {:id "site-7" :rigging-hazard-detected? true}})
        (is (= "USA-INR-000000" (get (last (store/installation-record-log-history s)) "record_id"))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (store/with-sites s {"x" {:id "x" :name "n" :jurisdiction "JPN"
                              :site-verified? true :lift-plan-approved? true
                              :safety-concern-unresolved? false :installation-notice-lead-days-actual 35
                              :status :intake}})
